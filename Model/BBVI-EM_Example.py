...
def bbvi_Estep(logprob, x_obs, model_params, var_params):
                                          
    def unpack_params(var_params_i):
        mu_lamb_i, log_sig_lamb_i = var_params_i
        return mu_lamb_i, log_sig_lamb_i
    
    def variational_objective(var_params, t):
        ...
        for i in range(len(x_obs)):
            ...
            lower_bound_sum += lower_bound / len(x_obs)
...
def bbvi_Mstep(logprob, x_obs, model_params, var_params):
...
def bbvi_simple(logprob, x_obs, model_params, var_params):
    
    def unpack_params(var_params_i):
        mu_lamb_i, log_sig_lamb_i = var_params_i
        return mu_lamb_i, log_sig_lamb_i
    
    def variational_objective(all_params, t):
        """Provides a stochastic estimate of the variational lower bound."""
        var_params   = all_params["var_params"]
...
def main(): 
    ...
    for e in range(Epochs):
        ...
        objective, gradient, _ = bbvi_Estep(log_p_zx_density, x_obs, model_params, var_params)
        init_var_params = var_params
        var_params_i_next = adam(gradient, init_var_params, step_size=step_size, num_iters=num_iters)
        var_params = var_params_i_next
        
        # M-Step
        print("({}) Optimizing model parameters...".format(e))          
        objective2, gradient2, _ = bbvi_Mstep(log_p_zx_density, x_obs, model_params, var_params)
    ...
    # 对照: 同时优化两者
    all_params = dict()                            
    all_params["var_params"] = var_params
    all_params["model_params"] = model_params
    for e in tqdm(range(Epochs)):
        objective, gradient, _ = bbvi_simple(log_p_zx_density, x_obs, model_params, var_params)
        all_params_init = all_params
        all_params_next = adam(gradient, all_params_init, step_size=step_size, num_iters=num_iters)
        all_params = all_params_next
    evaluate_on_simulation_data(x_obs, all_params["var_params"], all_params["model_params"], model_params_real)